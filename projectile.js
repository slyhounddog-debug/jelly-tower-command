...
export default class Projectile {
    ...
    init(game, x, y, angle, damage, range, origin, speed, radius = 15, gummyImpactCount = 0, popRockCount = 0, bubbleGumCount = 0, fireDamageCount = 0, chainBounceCount = 0, isAutoTurret = false, isCrit = false) {
        this.game = game;
    ...
    update(tsf) {
        ...
        for (const type in this.game.enemyPools) {
            this.game.enemyPools[type].forEach(m => allEnemies.push(m));
        }
        if (this.game.boss) {
            ...
            if (dist < this.radius + m.width / 2) {
                ...
                if (this.chainBounceCount > 0 && this.hitEnemies.length <= this.chainBounceCount) {
                    ...
                    for (const enemy of allEnemies) {
                        if (!this.hitEnemies.includes(enemy.id)) {
        ...
        const offScreen = this.y > this.game.PLAYABLE_AREA_HEIGHT || this.y < 0 || this.x < 0 || this.x > this.game.width;
    ...
    draw(ctx) {
        ...
        const numStripes = 4;
        const stripeAngle = Math.PI / 8;
        for (let i = 0; i < numStripes; i++) {
            ctx.fillStyle = (i % 2 === 0) ? '#e74c3c' : '#ff9ff3';
